...
const dbName = "beerdb.db"; // dbName = ':memory:'; //Pour travailler en mÃ©moirec
//const dbName =  ':memory:';
  ...
  new Promise((resolve, reject) => {
    ...
    db.serialize(() => {
      db.run(sqlCategorie, [], err => {
        if (err) {
          reject(err);
        } else console.log("Categorie créée");
      })
        .run(sqlStyle, [], err => {
          if (err) {
            reject(err);
          } else console.log("Style créée");
        })
        .run(sqlBrewery, [], err => {
          if (err) {
            reject(err);
          } else console.log("Brewery créée");
        })
        .run(geoCode, [], err => {
          if (err) {
            reject(err);
          } else console.log("Geocode crcréée");
        })
        .run(beer, [], err => {
          if (err) {
            reject(err);
